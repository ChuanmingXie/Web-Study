...
function shopcartInit() {
  ...
  for (var i = 0; i < arrayimgs.length; i++) {
    elememt += '<li><a href="#"> <img src="' + "./" + 'img/goodadvise/' + arrayimgs[i] + '" alt=""/></a>' + '<p>' + arrayamount[i] + '件<a href="#"><span class="orange floatright">删除</span></a></p>' + '</li>';
  }
...
function looksearchAgain() {
  ...
  for (var i = 0; i < arrayimgs.length; i++) {
    element += '<li><img src="' + "./" + 'img/gooddetail/' + arrayimgs[i] + '"title="' + introduce[i] + '"><p>' + introduce[i] + '</p></li>';
  }
