...
function arrayProperty() {
  var movies = new Array('飓风营救', '复仇者联盟3:无限战争', '疯狂的麦克斯:狂暴之路', '分歧者:绝地反击');
  ...
  switch (movies.constructor) {
    ...
    case Array:
      confirm('movies是一个数组对象');
      break;

    case Date:
      confirm('movies是一个时间对象');
      break;

    default:
      confirm('movies不是内置对象');
      break;
  }
}
...
function arrayjoin() {
  ...
  var showFoodsByJoin = function showFoodsByJoin(foods) {
  ...
  foods[1] = '肉夹馍';
  foods.push('潍坊火烧');
  var length = foods.push('泰山包子'); // document.write('<hr>排序前的数组长度：' + foods.length);
  ...
  showFoodsByJoin(foods);
  var betterfood = foods.slice(1, 4); //document.write('<hr>获赞的食品：' + betterfood);

  document.getElementsByClassName('resultMethod')[0].innerHTML += '<hr>获赞的食品有：' + betterfood; //document.write('<hr>获赞反向的：' + betterfood.reverse());

  document.getElementsByClassName('resultMethod')[0].innerHTML += '<hr>获赞逆向显示：' + betterfood.reverse();
  var prices = [35, 82, 20, 16, 55]; //document.write('<hr>排序前的数组：' + prices);

  document.getElementsByClassName('resultMethod')[0].innerHTML += '<hr>排序前的数组：' + prices; //document.write('<hr>默认的默认方式：' + prices.sort());

  document.getElementsByClassName('resultMethod')[0].innerHTML += '<hr>默认排序方式：' + prices.sort(); //document.write('<hr>指定排序方式：' + prices.sort(sortNumber));
...
function arrayMatrix() {
  ...
  var movies = [['科幻', '流浪地球', 57.9], ["动漫", "雾山五行", 27.9], ["战争", "长津湖", 87.9], ["历史", "我和我的父辈", 97.9]];
  var htmlstr = ''; // document.write('<table border="1px" rules="all">');

  htmlstr += '<table border="1px" rules="all">'; // document.write("<tr>");
