...
var app = new Vue({
  ...
  computed: {
    ...
    rowspanNum: function rowspanNum() {
      return function (i) {
  ...
  methods: {
    handleReduce: function handleReduce(i, j) {
      if (this.list[i].product[j].count == 1) return;
      this.list[i].product[j].count--;
    },
    handleAdd: function handleAdd(i, j, num) {
      this.list[i].product[j].count += num;
    },
    handleRemove: function handleRemove(i, j) {
    ...
    AddChecked: function AddChecked(e) {
      var that = this;
      $.each($(e.target).parents("tbody").children().children().has("input[type=checkbox]"), function (i, element) {
        ...
        if ($(element).children().prop('checked')) {
          ...
          var indexdel = that.checkItem.findIndex(function (item) {
            return item == i;
          });
