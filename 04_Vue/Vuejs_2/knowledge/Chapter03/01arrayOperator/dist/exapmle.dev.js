...
/* 3-1objectArray-operator-Vue数组对象操作 */
// Vue 使用Computed传递参数
var app = new Vue({
  el: '#app1',
  data: {
    item: {
      ...
      photo: ["https://tse3-mm.cn.bing.net/th/id/OIP-C.nfC2tVNM9TgwQ5QuqECd6wHaFj?pid=ImgDet&rs=1", "https://scpic.chinaz.net/files/pic/pic9/202009/apic27858.jpg"]
    }
  },
  computed: {
...
var app1 = new Vue({
  ...
  computed: {},
  methods: {
    clickMyself: function clickMyself(e) {
...
var app2 = new Vue({
  ...
  computed: {},
  methods: {
    methodIncludes: function methodIncludes(arr, info) {
    ...
    sortData: function sortData() {
      this.item.arrList.sort(function (a, b) {
        return a.count - b.count;
      });
    },
    reverserData: function reverserData() {
...
function arrayDelete() {
  ...
  console.log(list);
  var deleteIndex = list.findIndex(function (item) {
    return item.id === '2';
  });
...
function DuplicateArray() {
  ...
  console.log(arr.join(',')); // 1.利用Set()+Array.from()

  console.log(Array.from(new Set(arr))); // 2.利用两层循环+数组的splice方法

  console.log(removeDuplicateSplice(arr)); // 3.利用数组的indexOf方法

  console.log(removeDuplicateIndexOf(arr)); // 4.利用数组的includes方法

  console.log(removeDuplicateIncludes(arr)); // 5.利用数组的filter()+indexOf()

  console.log(removeDuplicateFilter(arr)); // 6.利用Map()

  console.log(removeDuplicateMap(arr)); // 7.利用对象
